...
def run(rl_core, model, total_eps=2, message=True, render=False, is_train=True,resolution=8
        img=None, points=None, gif_path="out/", gif_name="img_99.img"):
    gif_path = gif_path + gif_name[4:7] + '/'
    if not os.path.exists(gif_path):
        os.makedirs(gif_path)
    images = []
    env = SnakeEnv(img, model)
    iou_score = 0
...
if __name__ == "__main__":
    ...
    for i_batch, sample in enumerate(test_dataloader):
        iouscore = run(rl_core, total_eps=1, is_train=False, resolution=16,img=sample['img'].numpy().squeeze(),
